...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self.manager.requestSerializer setValue:@"application/x-www-form-urlencoded; application/json; charset=utf-8" forHTTPHeaderField:@"Content-Type"];
...
- (void)requestWithMethod:(HTTPRequestMethod)method path:(NSString *)path params:(NSDictionary *)params prepare:(PrepareBlock)prepare success:(SuccessBlock)success failure:(FailureBlock)failure {
    ...
    if (self.isReachable) {
        ...
        NSString *urlStr = [SERVER_HOST stringByAppendingString:path];
